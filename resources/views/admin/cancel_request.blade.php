            ...
            <div class="row min-vh-100 bg-black">
                      ...
                      <td>
                        <input type="hidden" name="item_id" value="{{ $result->id }}">
                          ...
                          <div class="form-check form-switch">
                              <input class="form-check-input cancel_status-toggle" type="checkbox" role="switch" id="cancel_statusToggle{{ $result->id }}" >
                              <!-- Label for the switch -->
                              <label class="form-check-label" for="cancel_statusToggle{{$result->id }}"></label>
                          </div>
                      </td>
              ...
              <script>
                document.addEventListener('DOMContentLoaded', function() {
                    var toggleSwitches = document.querySelectorAll('.cancel_status-toggle');
                    toggleSwitches.forEach(function(toggleSwitch) {
                        toggleSwitch.addEventListener('change', function() {
                            var productID = toggleSwitch.closest('tr').querySelector('[name="item_id"]').value;
                            var isFeature = toggleSwitch.checked ? 2 : 1; // Convert true/false to 1/0
                            var data = {
                                _token: '{{ csrf_token() }}',
                                cancel_status: isFeature
                            };
                            axios.patch('/products/' + productID + '/toggle-cancel', data)
